...
public class PlayerController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        switch (GameManager.Instance.m_PlayMode)
        {
            ...
            case PlayMode.HumanoidRobot:
                ...
                if (m_ArmL.GetComponent<Rocket>().m_State == RocketState.Idle && m_ArmR.GetComponent<Rocket>().m_State == RocketState.Idle && (Input.GetButtonDown("Combine") || m_Energy <= 0))
                {
                    m_Energy = 0;
    ...
    {
        if (m_IsBeamShooting || m_Energy < (6.25f + 0.5f) || (m_ArmL.GetComponent<Rocket>().m_State != RocketState.Idle && m_ArmR.GetComponent<Rocket>().m_State != RocketState.Idle)) { yield break; }
        m_IsBeamShooting = true;
        m_Energy -= 6.25f;
        yield return new WaitForSeconds(0.5f);
        if (m_ArmL.GetComponent<Rocket>().m_State == RocketState.Idle)
        {
            m_ArmL.SetActive(true);
            m_ArmL.GetComponent<Rocket>().TryFire();
        }
        else
        {
            m_ArmR.SetActive(true);
            m_ArmR.GetComponent<Rocket>().TryFire();
        }
