...
public class ElectricEffect : MonoBehaviour
{
	...
	void Update()
	{
		transform.localScale = new Vector3(Vector3.Distance(m_L.position, m_R.position),3, 3);
		transform.position = Vector3.Lerp(m_L.position, m_R.position, 0.5f) + transform.up * 0.5f;
